...
func (tq *TaskQuery) QueryModel() *ModelQuery {
	query := (&ModelClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) WithModel(opts ...func(*ModelQuery)) *TaskQuery {
	query := (&ModelClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withModel; query != nil {
		if err := tq.loadModel(ctx, query, nodes, nil,
			func(n *Task, e *Model) { n.Edges.Model = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadTaskRecords(ctx context.Context, query *TaskRecordQuery, nodes []*Task, init func(*Task), assign func(*Task, *TaskRecord)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(taskrecord.FieldTaskID)
	}
...
func (tq *TaskQuery) loadModel(ctx context.Context, query *ModelQuery, nodes []*Task, init func(*Task), assign func(*Task, *Model)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		fk := nodes[i].ModelID
...
func (tq *TaskQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withModel != nil {
			_spec.Node.AddColumnOnce(task.FieldModelID)
		}
	}
